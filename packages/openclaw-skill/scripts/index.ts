...
const MAX_USD_PER_CALL = parseFloat(process.env.WEB3ADS_MAX_USD_PER_CALL || '1.00');
const MAX_USD_PER_DAY = parseFloat(process.env.WEB3ADS_MAX_USD_PER_DAY || '10.00');
...
const tools: Record<string, {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        return {
          ...
          billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_check_balance', timestamp: new Date().toISOString() },
        };
      } catch (error) {
  ...
  web3ads_get_earnings: {
    description: 'Get detailed earnings breakdown from viewing and publishing ads',
    requiresExecution: false,
    handler: async (input) => {
      const start = Date.now();
      const walletAddress = (input.wallet_address as string) || WALLET_ADDRESS;
      ...
      try {
        ...
        return {
          ...
          billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_get_earnings', timestamp: new Date().toISOString() },
        };
      } catch (error) {
  ...
  web3ads_platform_info: {
    ...
    handler: async () => {
      ...
      return {
        ...
        data: {
          name: 'Web3Ads',
          description: 'Decentralized advertising platform where users earn crypto from viewing and publishing ads',
          network: 'Base Sepolia (Testnet)',
          currency: 'ETH (native)',
          features: [
        ...
        billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
        meta: { latency_ms: Date.now() - start, tool: 'web3ads_platform_info', timestamp: new Date().toISOString() },
      };
    },
  },
    ...
    handler: async (input) => {
      ...
      try {
        ...
        return {
          ...
          data: {
            ...data,
            description: 'Web3Ads implements x402 payment protocol. Pay for APIs using your ad earnings!',
          },
          billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_payment_info', timestamp: new Date().toISOString() },
        };
      } catch (error) {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        return {
          ...
          data,
          billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_list_campaigns', timestamp: new Date().toISOString() },
        };
      } catch (error) {
  ...
  web3ads_budget_status: {
    ...
    handler: async () => {
      ...
      return {
        ...
        billing: { estimated_cost_usd: 0, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
        meta: { latency_ms: Date.now() - start, tool: 'web3ads_budget_status', timestamp: new Date().toISOString() },
      };
    },
  },
  ...
  web3ads_create_campaign: {
    description: 'Create and fund an ad campaign using Web3Ads balance (x402 payment)',
    requiresExecution: true,
    handler: async (input) => {
      const start = Date.now();
      
      if (!ENABLE_EXECUTION) {
        return { ok: false, error: 'Execution not enabled. Set WEB3ADS_ENABLE_EXECUTION=true' };
      }
      ...
      const { name, description, ad_type, media_url, target_url, budget, category } = input as {
        name?: string;
        description?: string;
        ad_type?: string;
        media_url?: string;
      ...
      if (!name || !ad_type || !media_url || !target_url || !budget) {
        return { ok: false, error: 'Required: name, ad_type, media_url, target_url, budget' };
      }

      // Check budget limits
      const costUsd = budget * 2000; // ETH to USD
      if (costUsd > MAX_USD_PER_CALL) {
        return { ok: false, error: `Budget ${costUsd} USD exceeds per-call limit ${MAX_USD_PER_CALL} USD` };
      }
      if (dailySpent + costUsd > MAX_USD_PER_DAY) {
        return { ok: false, error: `Would exceed daily limit. Spent: ${dailySpent}, Limit: ${MAX_USD_PER_DAY}` };
      }
      ...
      try {
        ...
        return {
          ...
          billing: { estimated_cost_usd: costUsd, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_create_campaign', timestamp: new Date().toISOString() },
        };
      } catch (error) {
  ...
  web3ads_pay_x402: {
    ...
    handler: async (input) => {
      ...
      if (costUsd > MAX_USD_PER_CALL) {
        return { ok: false, error: `Amount ${costUsd} USD exceeds per-call limit ${MAX_USD_PER_CALL} USD` };
      }
      if (dailySpent + costUsd > MAX_USD_PER_DAY) {
        return { ok: false, error: `Would exceed daily limit. Spent: ${dailySpent}, Limit: ${MAX_USD_PER_DAY}` };
      }
      ...
      try {
        ...
        if (!response.ok) {
          return { ok: false, error: (data as { error?: string }).error || 'Payment failed' };
        }
        ...
        return {
          ...
          billing: { estimated_cost_usd: costUsd, daily_spent_usd: dailySpent, daily_limit_usd: MAX_USD_PER_DAY },
          meta: { latency_ms: Date.now() - start, tool: 'web3ads_pay_x402', timestamp: new Date().toISOString() },
        };
      } catch (error) {
...
async function main() {
  ...
  if (inputJson) {
    try {
      ...
      console.log(JSON.stringify({ ok: false, error: 'Invalid JSON input' }, null, 2));
      process.exit(1);
    }
  }
